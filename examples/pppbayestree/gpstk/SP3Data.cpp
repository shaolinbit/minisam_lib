...
throw(exception, FFStreamError, StringException)
{
    ...
    while(1)
    {
        ...
        else if(strm.lastLine[0] == 'P' || strm.lastLine[0] == 'V')
        {
            ...
            if(isVerC)
            {
                ...
                if(RecType == 'P')                                    // P flags
                {
                    ...
                    if(strm.lastLine[74] == 'E')
                        clockEventFlag = true;
                    if(strm.lastLine[75] == 'P')
                        clockPredFlag = true;
                    if(strm.lastLine[78] == 'M')
                        orbitManeuverFlag = true;
                    if(strm.lastLine[79] == 'P')
                        orbitPredFlag = true;
                }
            }
        }
        ...
        else              // normal flow
        {
            ...
            catch(FFStreamError& err)
            {
                ...
                if(what == string("text 0:Unexpected EOF"))
                {
                    ...
                    if(strm.lastLine.size() < 3)
                        status = 3;  // nothing there, quit
                    else
                        status = 0;  // go back and process it
                }
                else
                    GPSTK_RETHROW(err);
            }
        ...
        if(status == 3)
            break;  // quit if found EOF or EP|EV was processed, but
