...
int GetCarrierBand(const RinexObsID& roi)
{
    ...
    if(roi.band == ObsID::cbL2)
        return 2;
    if(roi.band == ObsID::cbG2)
        return 2;
    if(roi.band == ObsID::cbB1)
        return 2;   // TD this is not correct

    if(roi.band == ObsID::cbL5)
        return 5;

    if(roi.band == ObsID::cbE6)
        return 6;
    if(roi.band == ObsID::cbB3)
        return 6;

    if(roi.band == ObsID::cbE5b)
        return 7;

    if(roi.band == ObsID::cbE5ab)
        return 8;
                                  ...
                                  const RinexSatID& sat)
{
    if(sat.system==SatID::systemGPS)
    {
        ...
        if(rot == RinexObsHeader::C1)
            return TypeID::C1;
        if(rot == RinexObsHeader::P1)
            return TypeID::P1;
        if(rot == RinexObsHeader::L1)
            return TypeID::L1;
        if(rot == RinexObsHeader::D1)
            return TypeID::D1;
        if(rot == RinexObsHeader::S1)
            return TypeID::S1;
        // For L2: C2 P2 L2 D2 S2
        if(rot == RinexObsHeader::C2)
            return TypeID::C2;
        if(rot == RinexObsHeader::P2)
            return TypeID::P2;
        if(rot == RinexObsHeader::L2)
            return TypeID::L2;
        if(rot == RinexObsHeader::D2)
            return TypeID::D2;
        if(rot == RinexObsHeader::S2)
            return TypeID::S2;
        // For L5: C5 L5 D5 S5
        if(rot == RinexObsHeader::C5)
            return TypeID::C5;
        if(rot == RinexObsHeader::L5)
            return TypeID::L5;
        if(rot == RinexObsHeader::D5)
            return TypeID::D5;
        if(rot == RinexObsHeader::S5)
            return TypeID::S5;
    }
    ...
    else if(sat.system==SatID::systemGalileo)
    {
        ...
        if(rot == RinexObsHeader::S5)
            return TypeID::S5;
        // E5b
        if(rot == RinexObsHeader::C7)
            return TypeID::C7;
        if(rot == RinexObsHeader::L7)
            return TypeID::L7;
        if(rot == RinexObsHeader::D7)
            return TypeID::D7;
        if(rot == RinexObsHeader::S7)
            return TypeID::S7;
        // E5a+b
        if(rot == RinexObsHeader::C8)
            return TypeID::C8;
        if(rot == RinexObsHeader::L8)
            return TypeID::L8;
        if(rot == RinexObsHeader::D8)
            return TypeID::D8;
        if(rot == RinexObsHeader::S8)
            return TypeID::S8;
        // E6
        if(rot == RinexObsHeader::C6)
            return TypeID::C6;
        if(rot == RinexObsHeader::L6)
            return TypeID::L6;
        if(rot == RinexObsHeader::D6)
            return TypeID::D6;
        if(rot == RinexObsHeader::S6)
            return TypeID::S6;
    }
    ...
    if(sat.system==SatID::systemGPS)
    {
        ...
        if(roi.band==ObsID::cbL1)
        {
            ...
            if(roi.type == ObsID::otPhase)
                return TypeID::L1;
            if(roi.type == ObsID::otDoppler)
                return TypeID::D1;
            if(roi.type == ObsID::otSNR)
                return TypeID::S1;
        }
        // For L2: C2 P2 L2 D2 S2
